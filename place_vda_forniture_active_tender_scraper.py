...
class TenderScraper:
    ...
    def get_all_tenders_from_listing(self):
        ...
        for page_num in range(1, max_pages + 1):
            ...
            params = dict(self.listing_filters)
            if page_num > 1:
                params["page"] = page_num

            # Prepare only for console display. Requests will safely encode the
            # same parameters when the page is downloaded.
            display_url = requests.Request(
                "GET", self.listing_url, params=params
            ).prepare().url
            print(f"  URL: {display_url}")

            try:
                response = self.session.get(
                    self.listing_url, params=params, timeout=30
                )
...
def main():
    ...
    print("=" * 80)
    print("\nSource:")
    print("  https://place-vda.aflink.it/portalegare/index.php/bandi?scaduti=no&tipobando=FORNITURE")
    print("\nThis script will:")
    print("  1. Fetch only NON-EXPIRED FORNITURE tenders from every listing page")
